...
export default function Dashboard() {
  ...
  if (error) return (
    <div style={{ color: '#e74c3c', textAlign: 'center', marginTop: '40px' }}>
      <p>Failed to load dashboard. The server may be waking up.</p>
      <button onClick={fetchDashboard} disabled={retrying} style={{ padding: '8px 16px', background: retrying ? '#555' : '#1db954', color: '#000', border: 'none', borderRadius: '6px', cursor: 'pointer', marginBottom: '10px' }}>
        {retrying ? 'Retrying...' : 'Retry'}
      </button>
      <p><Link to="/login" style={{ color: '#1db954' }}>Log out and log in again</Link></p>
    </div>
  );
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
              ...
              {data.recentStreams?.map((stream, i) => (
                <motion.div key={stream._id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div>{stream.song?.title} - {new Date(stream.createdAt).toLocaleDateString()}</div>
                </motion.div>
              ))}
