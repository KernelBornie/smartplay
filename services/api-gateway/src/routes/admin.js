...
router.post('/approve-song', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    const { songId } = req.body;
    if (!songId) return res.status(400).json({ error: 'songId is required' });
    const song = await Song.findByIdAndUpdate(songId, { approvalStatus: 'approved' }, { new: true });
...
router.post('/reject-song', authenticate, authorize('admin'), async (req, res, next) => {
  try {
    const { songId } = req.body;
    if (!songId) return res.status(400).json({ error: 'songId is required' });
    const song = await Song.findByIdAndUpdate(songId, { approvalStatus: 'rejected' }, { new: true });
    if (!song) return res.status(404).json({ error: 'Song not found' });
...
// GET /admin/songs
router.get('/songs', authenticate, authorize('admin'), async (req, res, next) => {
  try {
...
router.delete('/songs/:id', authenticate, authorize('admin'), async (req, res, next) => {
  try {
...
// POST /admin/ban-user
router.post('/ban-user', authenticate, authorize('admin'), async (req, res, next) => {
  try {
